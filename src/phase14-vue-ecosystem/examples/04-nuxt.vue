// Nuxt 3 详解
// 本文件为参考代码，需在 Nuxt 项目中使用
// 运行: node 04-nuxt.vue (查看概念讲解)

console.log("=== Nuxt 3 详解 ===\n");

// ========== 1. 项目创建与结构 ==========
console.log("1. 项目创建与目录结构");
console.log(`
  ...
  │ server/api/      │ API 路由                           │
  │ public/          │ 静态资源                           │
  │ assets/          │ 需构建的资源 (CSS/图片)             │
  │ utils/           │ 工具函数 (自动导入)                │
  └──────────────────┴──────────────────────────────────┘
`);

// ========== 2. 文件路由 ==========
console.log("2. 文件路由系统");
console.log(`
  pages/ 目录自动生成路由:
...
// ========== 3. 数据获取 ==========
console.log("3. 数据获取 (四种方式)");
console.log(`
  // 1. useFetch — 推荐，自动缓存 + 去重
  const { data, pending, error, refresh } = await useFetch('/api/posts', {
    query: { page: 1 },
    pick: ['id', 'title'],           // 只返回部分字段 (减少传输)
    transform: (data) => data.items,  // 转换数据
    watch: [page],                    // 监听变化自动刷新
  });

  // 2. useAsyncData — 自定义数据源
  const { data } = await useAsyncData('posts', () => {
    return $fetch('/api/posts');
  });

  // 3. useLazyFetch — 不阻塞页面渲染
  const { data, pending } = useLazyFetch('/api/posts');
  // 页面立即显示，数据加载中显示 loading

  // 4. $fetch — 直接请求 (不缓存)
  const data = await $fetch('/api/posts');

  // SSR 注意: useFetch/useAsyncData 自动处理服务端到客户端的数据传递
  // 避免客户端重复请求!
`);

// ========== 4. 服务端 API (Nitro) ==========
console.log("4. Server API (Nitro)");
console.log(`
  ...
  export default defineEventHandler(async (event) => {
    ...
    return { posts, total: await db.post.count() };
  });
...
console.log(`
  ...
  // middleware/log.global.ts (全局中间件, .global 后缀)
...
console.log(`
  ...
  渲染模式对比:
  ┌──────┬────────────────────────────────────┐
  │ SSR  │ 每次请求在服务器渲染，SEO 好        │
  │ SSG  │ 构建时生成静态 HTML，性能最好       │
  │ ISR  │ SSG + 定时更新，平衡性能与新鲜度   │
  │ SPA  │ 纯客户端渲染，无需服务器            │
  └──────┴────────────────────────────────────┘
`);
